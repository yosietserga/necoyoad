{{--
    New Necoyoad — Product Page Template (choroni theme)
    Same widget-driven layout as home, but session object_type = 'product'
    so per-product widgets are loaded.
--}}
<x-layouts.storefront>
    @php $templateType = 'product'; @endphp
    ...
    <div id="mainContentContainer" nt-editable>
        <div class="row">
            ...
            @if (!empty($widgets['column_left']))
                <div class="large-3 medium-3 small-12"><div id="columnLeft" nt-editable><x-layouts.widget-row :position="$position" /></div></div>
            @endif
            ...
            @if (!empty($widgets['column_right']))
                <div class="large-3 medium-3 small-12"><div id="columnRight" nt-editable><x-layouts.widget-row :position="$position" /></div></div>
            @endif
        </div>
    </div>
