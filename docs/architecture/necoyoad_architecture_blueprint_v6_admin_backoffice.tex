...
PHP files across 21 controller folders plus the \texttt{AdminController} base class file. The \texttt{AdminController} base
...
\begin{itemize}[leftmargin=1.6em,itemsep=2pt]
  ...
  \item The admin \texttt{map.php} preloader --- the 843-line route-aware
        dependency switch.
  \item The 21 admin controller folders plus the \texttt{AdminController} base class (443 PHP files total).
\end{itemize}
...
The admin has 21 controller folders plus the \texttt{AdminController} base class file, containing 443 PHP files total:
...
\midrule
\texttt{admincontroller.php} & 1   & The AdminController base class (root file, not a folder). \\
\midrule
\textbf{Total}          & 443 & \\
\bottomrule
\end{tabularx}
...
With 443 PHP files across 21 controller folders plus the \texttt{AdminController} base
class, the admin is the most
