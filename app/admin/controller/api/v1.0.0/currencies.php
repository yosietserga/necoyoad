...
switch(strtolower($request_type)) {
    ...
    default:
        ...
        $total = $this->modelCurrency->getAllTotal($filters);
        $results = $this->modelCurrency->getAll($filters);

        foreach ($results as $l => $result) {
            $id = $result['currency_id'];
            $items[$l] = $result;
            $items[$l]['id'] = $id;
            $items[$l]['descriptions'] = $this->modelCurrency->getDescriptions($id);
        }
        ...
        $pagination->url = Url::createAdminUrl('api/v1/currencies') . $url . '&page={page}';
    ...
    case 'post':
        ...
        $id = $this->modelCurrency->add($this->prepareData('currencies', $this->request->post));
    ...
    case 'put':
        ...
        $query = $this->db->query("SELECT * FROM ". DB_PREFIX ."currency WHERE currency_id = '". (int)$this->request->getQuery('id') ."'");
        $query->row['sc'] = $this->request->getQuery('sc');
        $currency = $query->row;
        $this->request->post = json_decode(file_get_contents('php://input'), true);
        if ($currency['currency_id']) {
            $this->modelCurrency->update($currency['currency_id'], $this->prepareData('currencies', $currency));
