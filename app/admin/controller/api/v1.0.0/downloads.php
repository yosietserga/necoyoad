...
switch(strtolower($request_type)) {
    ...
    default:
        ...
        $total = $this->modelDownload->getAllTotal($filters);
        $results = $this->modelDownload->getAll($filters);

        foreach ($results as $l => $result) {
            $id = $result['download_id'];
            $items[$l] = $result;
            $items[$l]['id'] = $id;
            $items[$l]['stores'] = $this->modelDownload->getStores($id);
            $items[$l]['descriptions'] = $this->modelDownload->getDescriptions($id);
        }
        ...
        $pagination->url = Url::createAdminUrl('api/v1/downloads') . $url . '&page={page}';
    ...
    case 'post':
        ...
        $id = $this->modelDownload->add($this->prepareData('downloads', $this->request->post));
    ...
    case 'put':
        ...
        $query = $this->db->query("SELECT * FROM ". DB_PREFIX ."download WHERE download_id = '". (int)$this->request->getQuery('id') ."'");
        $query->row['sc'] = $this->request->getQuery('sc');
        $download = $query->row;
        $this->request->post = json_decode(file_get_contents('php://input'), true);
        if ($download['download_id']) {
            $this->modelDownload->update($download['download_id'], $this->prepareData('downloads', $download));
