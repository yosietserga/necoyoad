...
switch(strtolower($request_type)) {
    ...
    default:
        ...
        $total = $this->modelPost->getAllTotal($filters);
        $results = $this->modelPost->getAll($filters);
        ...
        $pagination->url = Url::createAdminUrl('api/v1/posts') . $url . '&page={page}';
    ...
    case 'post':
        ...
        $id = $this->modelPage->add($this->prepareData('posts', $this->request->post));
    ...
    case 'put':
        ...
        $query = $this->db->query("SELECT * FROM ". DB_PREFIX ."post WHERE post_id = '". (int)$this->request->getQuery('id') ."'");
        $query->row['sc'] = $this->request->getQuery('sc');
        $post = $query->row;
        $this->request->post = json_decode(file_get_contents('php://input'), true);
        if ($post['post_id']) {
            $this->modelPage->update($post['post_id'], $this->prepareData('posts', $post));
