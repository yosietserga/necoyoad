...
switch(strtolower($request_type)) {
    ...
    default:
        ...
        $total = $this->modelPage->getAllTotal($filters);
        ...
        $pagination->url = Url::createAdminUrl('api/v1/pages') . $url . '&page={page}';
    ...
    case 'post':
        ...
        $id = $this->modelPage->add($this->prepareData('pages', $this->request->post));
    ...
    case 'put':
        ...
        $query = $this->db->query("SELECT * FROM ". DB_PREFIX ."page WHERE post_id = '". (int)$this->request->getQuery('id') ."'");
        $query->row['sc'] = $this->request->getQuery('sc');
        $page = $query->row;
        $this->request->post = json_decode(file_get_contents('php://input'), true);
        if ($page['post_id']) {
            $this->modelPage->update($page['post_id'], $this->prepareData('pages', $page));
