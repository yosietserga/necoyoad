...
# Arvin Castro
# December 17, 2010
# https://codecri.me/sources/includes/class-xhttp-php/plugin-xhttp-cookie-php/
...
class xhttp_cookie {
	...
	public static function clear($name = '*', $website = '*', $directory = '/') {
		if($name == '*' and $website == '*' and $directory == '/') self::$datastore = [];
		else foreach(self::$datastore as $profile => &$domains) if($name == '*' or $profile == $name) {
			if($website == '*' and $directory == '/') self::$datastore[$name] = [];
			else foreach($domains as $domain => &$paths) if($website == '*' or stripos($domain, $website) + strlen($website) == strlen($domain)) {
					if($directory == '/') self::$datastore[$profile][$domain] = [];
					else foreach($paths as $path => &$cookies) if(stripos($path, $directory) === 0) self::$datastore[$profile][$website][$path] = [];
			}
		}
	}
