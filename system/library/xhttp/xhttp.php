...
# 9 September 2012
# https://codecri.me/sources/includes/class-xhttp-php/
...
class xhttp {
	...
	public static function request($url, $method = 'get', $requestData = array()) {
		...
		# Collect GET
		if($urlparts['query']) {
			if(!isset($requestData['get'])) $requestData['get'] = [];
		...
		if(isset($requestData['files'])) {
			...
			if(!is_array($requestData['curl'][CURLOPT_POSTFIELDS])) $requestData['curl'][CURLOPT_POSTFIELDS] = [];
	...
	# https://www.php.net/manual/en/function.http-parse-headers.php#77241
	...
	public function array_flatten_bfs($array) {
		$flat_array = (is_string($array)) ? array($array): [];
	...
	public static function runHook($name, &$hooks, $arguments) {
		...
		if(isset($hooks[$name])) $functions =& $hooks[$name]; else $functions = [];
