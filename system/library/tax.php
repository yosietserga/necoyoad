...
final class Tax {
	...
	public function __construct($registry) {
		...
		if ($this->session->has('country_id') && $this->session->has('zone_id')) {
			$country_id = $this->session->get('country_id');
			$zone_id = $this->session->get('zone_id');
		} else {
	...
	public function setZone($country_id, $zone_id) {
		...
		$this->session->set('country_id',$country_id);
		$this->session->set('zone_id',$zone_id);
	}
